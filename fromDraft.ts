...
export const ensureRicosContent = (
  content: RichContent | DraftContent
): RichContent => ('blocks' in content ? fromDraft(content) : content);
...
const cssToRicosDecoration = {
  ...
  'font-weight': (style: string) => {
    return {
      ...
      fontWeightValue: style === 'bold' ? 700 : 400,
    };
  },
...
const convertHeaderToInlineStyles = (styles) =>
  Object.entries(styles).map(([key, style]) =>
    cssToRicosDecoration[key](style)
  );
...
export const fromDraft = (
  ...
  const parseAtomicBlock = ({
    key,
    data,
    entityRanges,
  }: RicosContentBlock): Node | null => {
